...
void eraseConnectedBlockCount(int _x, int _y, int _cellType) {
	if (
		(_x < 0) || _x >= FIELD_WIDTH || (_y < 0) || (_y >= FIELD_HEIGHT)
		|| (cells[_y][_x] == CELL_TYPE_NONE)
		|| (cells[_y][_x] != _cellType)
		...
		return;
	cells[_y][_x] = CELL_TYPE_NONE;
	eraseConnectedBlockCount(_x, _y - 1, _cellType);
	eraseConnectedBlockCount(_x, _y + 1, _cellType);
	eraseConnectedBlockCount(_x + 1, _y, _cellType);
	eraseConnectedBlockCount(_x - 1, _y, _cellType);
}
void eraseConnectedBlockCountAll() {
	memset(checked, 0, sizeof checked);
	for (int y = 0; y < FIELD_HEIGHT; y++) {
		for (int x = 0; x < FIELD_WIDTH; x++) {
			int n = getConnectedBlockCount(x, y, cells[y][x], 0);
			if (n >= 3) {
				eraseConnectedBlockCount(x, y, cells[y][x]);
				locked = true;
			}
		}
	}
